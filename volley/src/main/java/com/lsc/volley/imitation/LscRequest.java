...
/**
 * Created by lsc on 2020/4/19 19:35.
 * E-Mail:2965219926@qq.com
 *
 * Comparable 用于比较对象的大小。重写compareTo
 */
public abstract class LscRequest<T> implements Comparable<LscRequest<T>> {

    /** POST或PUT参数 默认编码。看到{@link # getParamsEncoding ()}。 */
    ...
    /** 收到请求结果后回调 */
    /* package */ interface LscNetworkRequestCompleteListener {

        /** 收到网络响应时回调。 */
        void onResponseReceived(LscRequest<?> request, Response<?> response);

        /** 网络请求返回而没有有效响应时回调。 */
        void onNoUsableResponseReceived(LscRequest<?> request);
    }

    /** 跟踪此请求生存期的事件日志，调试使用。 */
    private final LscVolleyLog.MarkerLog mEventLog = LscVolleyLog.MarkerLog.ENABLED ? new LscVolleyLog.MarkerLog() : null;

    /**
     * 请求方式。当前支持GET,POST,PUT,DELETE,HEAD,OPTIONS,TRACE,PATCH
     */
    ...
    /** 此请求关联的请求队列 */
    private LscRequestQueue mRequestQueue;
    ...
    /** 此request是否以交付response */
    @GuardedBy("mLock")
    private boolean mResponseDelivered = false;

    /** 发生HTTP 5xx(服务器)错误时是否应重试请求。 */
    ...
    /**
     *当一个请求可以从缓存中检索，但必须从网络中刷新时，缓存条目将存储在这里，以便在出现“未修改”响应时，
     * 我们可以确保它没有从缓存中被清除。
     */
